...
\begin{itemize}
    ...
    \item The paper proposes characteristic guidance as a systematic solution to the large guidance scale issue by addressing the nonlinearity neglicted in classifier-free guidance. Characteristic guidance employs a correction term with a projection operation. This projection differences between pixel space diffusion, latent space diffusion and low dimensional cases.
    \item The non-linear corrections predominantly occur in the middle stages of the diffusion process.
\end{itemize}
...
\paragraph{Summary}
\begin{itemize}
    \item The authors disentangle the CFG update term into parallel and orthogonal components and show that the parallel component primarily causes oversaturation, while the orthogonal component enhances image quality.
    \item Rescaling and momentum is added to the CFG updates by the gradient ascent analogy. The rescaling is to control large update norms, which can cause drifts in the sampling process, they propose to constrain the updates within a sphere. The momentum is used in a repulsive way to down-weigh components arelady present in previous sampling steps, they call it reverse momentum.
    \item Adaptive projected guidance (APG) retains the quality-boosting advantages of CFG while preventing oversaturation at higher guidance scales.
\end{itemize}
